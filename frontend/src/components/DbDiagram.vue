...
onMounted(async () => {
  ...
  const tableGroups = svg
    .selectAll<SVGGElement, Table>('g.table-group')
    .data(tables)
    .enter()
    .append('g')
    ...
    .call(
      d3
        ...
        .on('drag', function (event: d3.D3DragEvent<SVGGElement, Table, unknown>, d) {
          d.x += event.dx
          d.y += event.dy
  ...
  // Per table group: render header dan kolom
  ...
  // Ambil koordinat tengah kolom
  function getColumnCenter(tableId: string, column: string, side: 'left' | 'right' | 'center' = 'center') {
    const svgRect = svgRef.value?.getBoundingClientRect()
    const node = d3.select<SVGRectElement, unknown>(`#${tableId}_${column}`).node()

    if (!svgRect || !node) return { x: 0, y: 0 }
